...
<body>
	<h2>Form Edit Events</h2>
	<form action="{{ url('event/'.$event->id) }}" method="POST">
		<input type="hidden" name="_token" value="{{ csrf_token() }}">
		{{ method_field('put') }}
		<div class="form-group row">
			<label for="" class="col-md-1">Title: </label>
			<input type="text" class="col-md-5" name="title" value="{{ old('title',isset($event) ? $event->title : null) }}">
		</div>
		<div class="form-group row">
			<label for="" class="col-md-1">Content: </label>
			<input type="text" class="col-md-5" name="content" value="{{ old('content',isset($event) ? $event->content : null) }}">
		</div>
		<div class="form-group row">
			<label for="" class="col-md-1">Time: </label>
			{{-- <input type="text" class="col-md-5" name="time" value="{{ old('time') }}"> --}}
			<div id="datepicker" class="input-group date" data-date-format="yyyy-mm-dd">
				<input class="form-control" name="time" readonly="" type="text" value="{{ old('time',isset($event) ? $event->time : null) }}">
		...
		<div class="form-group row">
			...
			<input type="text" class="col-md-5" name="location" value="{{ old('location',isset($event) ? $event->location : null) }}">
		</div>
